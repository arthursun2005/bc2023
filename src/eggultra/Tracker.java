...
//    static IslandState[][] islands = new IslandState[69][69];
    ...
    static MapLocation getClosestMine() {
        ...
        if (wells.size() == 0) return rc.getLocation();
...
//    }
    ...
    static MapLocation getOptimalWell() throws GameActionException
    {
        ...
        for (CustomWell well : wells) {
            ...
            if (rc.getRoundNum() <= 80 && rc.getMapWidth() <= 25 && rc.getMapHeight() <= 25 && rc.getType().equals(RobotType.CARRIER) && well.getResourceType() == ResourceType.ADAMANTIUM && rc.getID() % 3 != 0) {
                continue;
            }
    ...
    static final int POSSI = 20;

    static void writepossi(int[] x) throws GameActionException {
        if (!rc.canWriteSharedArray(0, 0)) return;
        rc.writeSharedArray(POSSI, (1-x[1]) + 2*(1-x[2]) + 4*(1-x[3]));
    }

    static int[] readpossi() throws GameActionException {
    ...
    // Indices [0, MAX_ENEMY_GROUP) store locations of enemy groups
    static final int ENEMY_GROUP_RADIUS = 34; // How wide is a group
    static final int ENEMY_GROUP_SEPARATION = 50; // How far apart groups are
    static final int ENEMY_GROUP_THRESHOLD = 3; // Minimum enemies in a group
    static final int MAX_ENEMY_GROUP = 20; // Number of groups being tracked

    static ArrayList<MapLocation> getEnemyGroups() throws GameActionException {
        ArrayList<MapLocation> enemyCoords = new ArrayList<>();
        for (int i=0; i<MAX_ENEMY_GROUP; i++) {
            if (rc.readSharedArray(i)==0) continue;
    ...
    static boolean addEnemyGroup() throws GameActionException {
        ...
        if (enemyCoords.size() == MAX_ENEMY_GROUP) return false;
        for (MapLocation group : enemyCoords) {
            if (pos.isWithinDistanceSquared(group, ENEMY_GROUP_SEPARATION)) return false;
        }
        for (int i=0; i<MAX_ENEMY_GROUP; i++) {
            if (rc.readSharedArray(i) == 0) {
                rc.writeSharedArray(i, pos.x * 69 + pos.y + ((rc.getRoundNum()/2) % 13) * 5000 + 1);
                break;
            }
        }
        return true;
    ...
    static void clearEnemyGroups() throws GameActionException {
        ...
        for (int i=0; i<MAX_ENEMY_GROUP; i++) {
            if (rc.readSharedArray(i) == 0) continue;
            if ((rc.readSharedArray(i) - 1) / 5000 == (rc.getRoundNum()/2) % 13) {
                int coord = (rc.readSharedArray(i) - 1) % 5000;
                int coordX = coord / 69;
                int coordY = coord % 69;
    ...
    /*static void removeEnemyGroup() throws GameActionException {
        if (!rc.canWriteSharedArray(0, 0)) {
            return;
        }
        MapLocation pos = rc.getLocation();
        int coordEncode = pos.x * 69 + pos.y + 1;
        for (int i=0; i<MAX_ENEMY_GROUP; i++) {
            if (rc.readSharedArray(i) == coordEncode) {
                rc.writeSharedArray(i, 0);
                break;
            }
        }
