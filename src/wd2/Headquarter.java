...
public class Headquarter extends Robot
{
    ...
    static boolean shouldMakeAmplifier = false;
    ...
    public MapLocation getOptimalSpawn(RobotType robot) throws GameActionException {
    ...
    public void run() throws GameActionException
    {
        ...
        rc.setIndicatorString(shouldMakeAmplifier ? "try to make amplifier" : "-");

        if (shouldMakeAmplifier || (roundNum >= 40 && rng.nextInt(15) == 0)) {
            shouldMakeAmplifier = true;
            if (ada >= 40 && mana >= 40) {
                shouldMakeAmplifier = false;
                toMake = RobotType.AMPLIFIER;
            }
        }
        else if (ada - mana >= 150)
        {
        ...
        {
            MapLocation loc = getOptimalSpawn(RobotType.LAUNCHER);
            made = tryMake(loc);
        }

        if (toMake.equals(RobotType.AMPLIFIER))
        {
            MapLocation loc = getOptimalSpawn(RobotType.AMPLIFIER);
            made = tryMake(loc);
        }
    }
}
