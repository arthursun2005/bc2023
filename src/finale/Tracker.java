...
public class Tracker {
    ...
    boolean shouldWrite() throws GameActionException {
        long f;
        f = wellX;
        boolean flag = false;
        while (f > 0) {
            ...
            f -= h;
            int x = Util.log2(h);
            if (wellB[x] != rB[x]) return false;
            if (wellB[x] > 0) flag = true;
        }
        return flag;
    }
    ...
    boolean hasEmpty(MapLocation well) throws GameActionException {
        for (Direction i : Direction.allDirections()) {
            MapLocation tmp = well.add(i);
            if (tmp.equals(rc.getLocation())) {
                return true;
            }
            if (rc.canSenseLocation(tmp)) {
                Direction cDir = rc.senseMapInfo(tmp).getCurrentDirection();
                if (!rc.sensePassability(tmp) || (cDir != Direction.CENTER && cDir != i.opposite())) {
                    //permanently false
                    taken.append("!" + (tmp.x + 10) + "" + (tmp.y + 10) + "" + 6900);
                    continue;
                }
                if (rc.canSenseRobotAtLocation(tmp)) {
                    //temporarily false
                    taken.append("!" + (tmp.x + 10) + "" + (tmp.y + 10) + "" + (rc.getRoundNum() + 1000));
                    continue;
                }
                return true;
            }
            else {
                int idx = taken.lastIndexOf("!" + (tmp.x + 10) + "" + (tmp.y + 10));
                if (idx != -1) {
                    int lastRound = Integer.valueOf(taken.substring(idx + 5, idx + 9)) - 1000;
                    //System.out.println("found " + well + " " + lastRound);
                    if (rc.getRoundNum() < lastRound + 20) {
                        continue;
                    }
                }
                return true;
        ...
        rc.setIndicatorString("ohno");
        return false;
    }

    boolean sosmall = false;
    ...
    MapLocation getBestWell(MapLocation threat) throws GameActionException {
        ...
        if (sosmall && rc.getRoundNum() < 75) ignoreAda = true;
