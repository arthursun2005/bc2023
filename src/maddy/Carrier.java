...
public class Carrier extends Robot {
    ...
    public void run() throws GameActionException {
        ...
        if (huhh >= 4 * GameConstants.CARRIER_CAPACITY / 5 && rc.getRoundNum() < 89)
            shouldReturn = true;

        if (ada >= 3 * GameConstants.CARRIER_CAPACITY / 4)
            shouldReturn = true;
            ...
            if (rc.isMovementReady()) {
                for (Direction dir : Direction.allDirections()) {
                    if (rc.canMove(dir)) {
                        ...
                        if (tmp.distanceSquaredTo(well) <= 2 && rc.senseMapInfo(tmp).getCurrentDirection() == Direction.CENTER) {
                            rc.move(dir);
                            break;
                        }
                    }
