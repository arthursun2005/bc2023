...
public class Attack {
    ...
    public int getEnemyWeaknessMetric(RobotInfo enemy, MapLocation HQLoc) throws GameActionException {
        ...
        int adjustedHealth = enemy.health * 20000 * 20 - HQLoc.distanceSquaredTo(enemy.location) * 20000 + rc.getID();
    ...
    public int getEnemyStrengthMetric(RobotInfo enemy) throws GameActionException {
        if (!enemy.type.equals(RobotType.LAUNCHER))
            return -1_000_000_000;
        int adjustedHealth = -rc.getLocation().distanceSquaredTo(enemy.location) * 20 + enemy.health;
        return adjustedHealth;
    }
    ...
    public int getStatus(MapLocation weakLoc) throws GameActionException {
        ...
        for (RobotInfo friend : friends) {
            ...
            if (weakLoc != null) {
                if (friend.location.distanceSquaredTo(weakLoc) < rc
                        .adjacentLocation(rc.getLocation().directionTo(weakLoc)).distanceSquaredTo(weakLoc))
                    // if (friend.location.distanceSquaredTo(weakLoc) <
                    // rc.getLocation().distanceSquaredTo(weakLoc))
                    ahead = true;
            }
        }
        for (RobotInfo enemy : enemies) {
