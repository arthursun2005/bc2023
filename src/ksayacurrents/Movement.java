...
public class Movement {
    ...
    boolean canMove(Direction desired) throws GameActionException {
        ...
        MapLocation tar = currentLocation.add(desired);
        ...
        if (currentLocation.distanceSquaredTo(actualTar) > 2) return true;
...
//                return getGreedyDirection();
    ...
    Direction normal(Direction togo) throws GameActionException {
        if (canMove(togo)) {
            rc.setIndicatorDot(rc.getLocation(), 69, 69, 255);
            if (rc.senseMapInfo(rc.getLocation().add(togo)).getCurrentDirection() == Direction.CENTER) {
                if (canMove(togo.rotateLeft()) && rc.senseMapInfo(rc.getLocation().add(togo.rotateLeft())).getCurrentDirection() != Direction.CENTER)
                    return togo.rotateLeft();
                if (canMove(togo.rotateRight()) && rc.senseMapInfo(rc.getLocation().add(togo.rotateRight())).getCurrentDirection() != Direction.CENTER)
                    return togo.rotateRight();
            }
            return togo;
        }
        return null;
...
//    }
    ...
    Direction tryMove(MapLocation currentTarget, Direction previous) throws GameActionException {
        if (rng == null) rng = new Random(rc.getID());
        if (invalid == null) invalid = new StringBuilder(String.format("%3690s",""));
        rc.setIndicatorString("bruh " + currentState);
