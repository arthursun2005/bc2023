...
public class Launcher extends Robot {
    ...
    public void randomizedGreedy(MapLocation target, int mul, int tol) throws GameActionException {
        ...
        for (Direction dir : directions) {
            ...
            if (hq != null && hq.distanceSquaredTo(loc) <= 9) {
                continue;
            }
    ...
    MapLocation prevBestieLoc = null;
        ...
        if (status == 1) {
            ...
            for (RobotInfo friend : friends) {
                if (friend.type == RobotType.LAUNCHER) {
                    ...
                    // lowerCount++;
                    count++;
                    if (friend.getLocation().distanceSquaredTo(rc.getLocation()) <= 8) ocount++;
                    else {
                        bestMove = friend.getLocation();
            ...
            if (bestie != null) {
                ...
                if (prevBestieLoc == null || !rc.canMove(prevBestieLoc.directionTo(bestie)) || rc.getLocation().distanceSquaredTo(bestie) > 5) moveTo(bestie);
                else {
                    rc.move(prevBestieLoc.directionTo(bestie));
                }
                prevBestieLoc = bestie;
            } else {
                if (ocount < 2 && bestMove != null) {
                    attack.tryAttack();
//                    attack.snipe();
                    moveTo(bestMove);
                    attack.tryAttack();
            ...
            if (rc.getRoundNum() % 3 != 0) {
                if (rc.getLocation().distanceSquaredTo(symmetry.update()) > 25) {
