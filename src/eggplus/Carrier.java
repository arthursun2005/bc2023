    ...
    public void run() throws GameActionException {
        ...
        if (rc.getAnchor() != null) {
            ...
            if (!mined && !rc.getLocation().isWithinDistanceSquared(target, 2)) {
                rc.setIndicatorString("Going to well " + target.toString() + " " + tracker.wells.size() + " "
                        + rc.getLocation().isWithinDistanceSquared(target, 1) + " " + Tracker.wells.size());
            ...
            rc.setIndicatorString("nothing to do " + target.toString() + " " + tracker.wells.size() + " " + mined + " "
                    + rc.getLocation().isWithinDistanceSquared(target, 1) + " " + rc.canCollectResource(target, 1) + " "
                    + rc.getActionCooldownTurns() + " " + (rc.getResourceAmount(ResourceType.ADAMANTIUM)
                            + rc.getResourceAmount(ResourceType.MANA) + rc.getResourceAmount(ResourceType.ELIXIR)));
            ...
            if (!mined && rc.getLocation().isWithinDistanceSquared(target, 2) && rc.canCollectResource(target, 1)) {
                ...
                holding = rc.getResourceAmount(ResourceType.ADAMANTIUM) + rc.getResourceAmount(ResourceType.MANA)
                        + rc.getResourceAmount(ResourceType.ELIXIR);
            ...
            if (mined) {
                ...
                rc.setIndicatorString("Returning to HQ " + tracker.wells.size() + " "
                        + rc.getLocation().isWithinDistanceSquared(current, 2) + " "
                        + (rc.getResourceAmount(ResourceType.ADAMANTIUM) + rc.getResourceAmount(ResourceType.MANA)
                                + rc.getResourceAmount(ResourceType.ELIXIR)));
