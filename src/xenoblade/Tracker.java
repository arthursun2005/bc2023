...
public class Tracker {
    ...
    boolean hasEmpty(MapLocation well) throws GameActionException  {
        int idx = taken.lastIndexOf("!" + (well.x + 10) + "" + (well.y + 10));
        if (idx != -1) {
        ...
        for (Direction i : Direction.allDirections()) {
            ...
            if (!rc.canSenseLocation(tmp) || tmp.equals(rc.getLocation()) || (rc.sensePassability(tmp) && !rc.canSenseRobotAtLocation(tmp) && rc.senseMapInfo(tmp).getCurrentDirection() == Direction.CENTER)) {
                return true;
            }
        }
        taken.append("!" + (well.x + 10) + "" + (well.y + 10) + "" + (rc.getRoundNum() + 1000));
        //System.out.println("!" + (well.x + 10) + "" + (well.y + 10) + "" + (rc.getRoundNum() + 1000));
        return false;
    }
