...
public class Carrier extends Robot {
    ...
    public void run() throws GameActionException {
        ...
        if (!agg && aggro < 5000 && creationRound >= aggro && aggro != 0 && gene < 20)
            agg = true;
        if (!agg && aggro >= 5000 && creationRound >= aggro - 5000 && gene < 50)
            agg = true;

        if (agg) {
            boolean ahead = false;
            rc.setIndicatorDot(rc.getLocation(), 255, 80, 255);
            ...
            if (gene % 5 == 1) {
                ...
                if (rc.getWeight() < 40 && adaWell != null) {
                    moveTo(adaWell);
                    moveTo(adaWell);
                    tryMine(adaWell);
                    tryMine(adaWell);
            ...
            rc.setIndicatorString(ahead ? "yes" : "nono");
        ...
        if (ada >= 3 * GameConstants.CARRIER_CAPACITY / 4 && rc.getRoundNum() < 179)
            shouldReturn = true;
