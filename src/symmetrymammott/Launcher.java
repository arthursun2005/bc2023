...
public class Launcher extends Robot {
    ...
    public void run() throws GameActionException {
        ...
        if (status == 1) {
            ...
            if (site != null && rc.getLocation().distanceSquaredTo(site) < 2 * rc.getLocation().distanceSquaredTo(target.loc)) {
                moveTo(site);
            ...
            if (enemyLocs.size()==0) {
                for (MapLocation hqLoc : tracker.HQLocs) {
                    ...
                    if (tracker.possi[3]==1) enemyLocs.add(new possiLoc(oppositeLoc,3,(hqLoc==parentLoc?6900:6900)));
                }
            }
